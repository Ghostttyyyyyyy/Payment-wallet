const nWallet = {
    ...
    newWallet: () => {
        ...
        valorTxt.textContent = `Rs${nWallet.blc}.00`;
        ...
        rsvdValue.textContent = `Rs${nWallet.rsvd}.00`;
        ...
        addBtn.addEventListener('click', function () {
            ...
            if (Number.isFinite(vl) && Math.sign(vl) != -1 && vl <= nWallet.limit) {
                ...
                alert(`Value exceeded the account limit for deposit. Limit: ${nWallet.limit}`)
            }
            valorTxt.textContent = `Rs${nWallet.blc.toFixed(2)}`;
        });
        ...
        wdwBtn.addEventListener('click', function () {
            ...
            if (Number.isFinite(vl) && Math.sign(vl) != -1 && vl <= nWallet.limit) {
                //additional validation to check if there's balance available
                if (vl > nWallet.blc) {
        ...
        svnBtn.addEventListener('click', function () {
            ...
            valorTxt.textContent = `Rs${nWallet.blc.toFixed(2)}`;
            rsvdValue.textContent = `Rs${nWallet.rsvd.toFixed(2)}`;
        });
        ...
        rsvdBtn.addEventListener('click', function () {
            ...
            if (Number.isFinite(vl) && Math.sign(vl) != -1) {
                //additional validation to check if there's reserve available
                if (vl > nWallet.rsvd) {
                    alert('Value not available');
                } else {
                    nWallet.rsvd -= vl;
        ...
        transferBtn.addEventListener('click', function () {
            let vl = parseFloat(Number(prompt('Enter the amount to transfer')));
            //check if the entry is a number and is positive
            if (Number.isFinite(vl) && Math.sign(vl) != -1) {
                //additional validation to check if there's balance available
                if (vl > nWallet.blc) {
                    alert('Insufficient balance');
                } else {
                    let recipient = prompt('Enter recipient username or account ID:');
                    // Perform the transfer
                    // For demonstration purposes, let's assume the transfer is successful
                    nWallet.blc -= vl;
                    alert(`Successfully transferred Rs${vl.toFixed(2)} to ${recipient}`);
                }
            }
            // Update displayed balances
            valorTxt.textContent = `Rs${nWallet.blc.toFixed(2)}`;
            // You might want to update the recipient's balance here if applicable
        });

        // add delete function
        trashIcon.addEventListener('click', function () {
            if (nWallet.blc != 0) {
                alert(`You need to empty the account first. Available: Rs${nWallet.blc.toFixed(2)}`)
            } else {
